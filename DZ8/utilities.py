...
def dice_metric(predicted, target):
    '''
    В этой функции мы берем «прогнозируемое» и «цель» (метку) для расчета значения коэффициента,
    а затем используем их для расчета значения метрики для обучения и проверки.
    '''
...
def calculate_weights(val1, val2):
    '''
    В этой функции мы берем количество пикселей фона и фона, чтобы вернуть «веса» для значений перекрестной энтропийной потери.
    '''
